...
TokenKind = Enum('TokenKind', [
    ...
    'CHARACTER', 'FLOAT', 'IDENTIFIER', 'NAT', 'KEYWORD', 'MULTI_KEYWORD', 'OPERATOR', 'LOW_PRECEDENCE_OPERATOR', 'STRING', 'SYMBOL',
...
def scanNextToken(state: ScannerState) -> tuple[ScannerState, Token]:
    ...
    ## Numbers
    if isDigit(c) or (c == b'-'[0] and isDigit(state.peek(1))):
        state.advance()
        ...
        if state.peek(0) == b':'[0]:
            ...
            if isOperatorCharacter(state.peek(0)):
                while isOperatorCharacter(state.peek(0)):
                    state.advance()
                return state, state.makeTokenStartingFrom(TokenKind.LOW_PRECEDENCE_OPERATOR, initialState)
