...
class TestParser(unittest.TestCase):
    ...
    def testKeywordApplication(self):
        node = self.parseSourceStringWithoutErrors("let: #x with: 42")
        self.assertTrue(node.isApplicationNode())
        self.assertTrue(node.functional.isIdentifierReferenceNode())
        self.assertEqual(node.functional.value, 'let:with:')

        self.assertEqual(len(node.arguments), 2)

        argument: ParseTreeNode = node.arguments[0]
        self.assertTrue(argument.isLiteralSymbolNode())
        self.assertEqual(argument.value, 'x')
    ...
    def testAssociation(self):
        ...
        self.assertTrue(node.isAssociationNode())

        element: ParseTreeLiteralIntegerNode = node.key
        self.assertTrue(element.isLiteralIntegerNode())
        self.assertEqual(element.value, 42)

        element: ParseTreeLiteralIntegerNode = node.value
        self.assertTrue(element.isLiteralIntegerNode())
    ...
    def testBlockSingleArgument(self):
    ...
    def testLexicalBlock(self):
        node = self.parseSourceStringWithoutErrors('{}')
        self.assertTrue(node.isLexicalBlockNode())
    ...
    def testBinaryExpressionSequence2(self):
    ...
    def testLowPrecedenceBinaryExpressionSequence(self):
        node = self.parseSourceStringWithoutErrors("1 ::+ 2")
        self.assertTrue(node.isMessageSendNode())

        term: ParseTreeNode = node.receiver
        self.assertTrue(term.isLiteralIntegerNode())
        self.assertEqual(term.value, 1)

        operator: ParseTreeNode = node.selector
        self.assertTrue(operator.isLiteralSymbolNode())
        self.assertEqual(operator.value, '+')

        term: ParseTreeNode = node.arguments[0]
        self.assertTrue(term.isLiteralIntegerNode())
        self.assertEqual(term.value, 2)
    ...
    def testCascadeMessageSend(self):
        ...
        message: ParseTreeCascadedMessageNode = node.messages[0]
        ...
        message: ParseTreeCascadedMessageNode = node.messages[1]
        self.assertTrue(message.selector.isLiteralSymbolNode())
        self.assertEqual(message.selector.value, '-')
        self.assertEqual(len(message.arguments), 1)
        ...
        message: ParseTreeCascadedMessageNode = node.messages[2]
        self.assertTrue(message.selector.isLiteralSymbolNode())
    ...
    def testEmptyDictionary(self):
        node = self.parseSourceStringWithoutErrors("#{}")
        self.assertTrue(node.isDictionaryNode())
        self.assertEqual(len(node.elements), 0)

    def testDictionary(self):
        node = self.parseSourceStringWithoutErrors("#{First: 1}")
        self.assertTrue(node.isDictionaryNode())
        self.assertEqual(len(node.elements), 1)

    def testDictionary2(self):
        node = self.parseSourceStringWithoutErrors("#{First: 1. Second: 2. Third:}")
        self.assertTrue(node.isDictionaryNode())
        self.assertEqual(len(node.elements), 3)
